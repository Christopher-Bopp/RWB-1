...
if ($action eq "login") { 
  if ($run) { 
    ...
    #
     my ($user,$password) = (param('user'),param('password'));
      $user=encryptDecrypt($user);
	 $password=encryptDecrypt($password);
    ...
    if (ValidUser($user,$password)) { 
      ...
      $outputcookiecontent=join("/",encryptDecrypt($user),encryptDecrypt($password));
      $action = "base";
      $run = 1;
    } else {
...
print "<title>Red, White, and Blue</title>";
  print "<script type=\"text/javascript\" src=\"rwb-encrypt.js\"> </script>";
print "</head>";
...
if ($action eq "login") { 
  ...
  if ($logincomplain or !$run) { 
    print start_form(-name=>'Login',-onSubmit=>'loginEncrypt()'),
...
my $key="1";
my @char1 = split("", $output);
my @char2= split("", $input);
for(my $i=0;$i<$size;$i++)
{
$char1[$i]=$char2[$i]^$key;
}
...
if ($action eq "give-opinion-data") { 
   if (!UserCan($user,"give-opinion-data")) { 
	...
	h2('Give Opinion'),
	  "Color: ", popup_menu(-name=>'color',-values=>['-1','0','1'],-labels=>{'-1'=>'Red','0'=>'White','1'=>'Blue'}),
	    p,
	      hidden(-name=>'run',-default=>['1']),
...
print "<p>".$count1.$count2."</p>";

#print textfield(-name=>'latne',-value=>$latne),
 #print textfield(-name=>'latne',-value=>$latne),
#	textfield(-name=>'longne',-value=>$longne),
#	textfield(-name=>'latsw',-value=>$latsw),
#	textfield(-name=>'longsw',-value=>$longsw);
